#ifndef LTBL_MATH_HPP
#define LTBL_MATH_HPP
...
namespace ltbl 
{
	...
	inline bool rectIntersects(const sf::FloatRect &rect, const sf::FloatRect &other){
	...
	inline sf::FloatRect rectExpand(const sf::FloatRect &rect, const sf::Vector2f &point) {
	...
	inline bool shapeIntersection(const sf::ConvexShape &left, const sf::ConvexShape &right) {
		...
		for (unsigned int i = 0; i < left.getPointCount(); i++)
		...
		for (unsigned int i = 0; i < right.getPointCount(); i++)
		...
		for (unsigned int i = 0; i < left.getPointCount(); i++) {
			...
			for (unsigned int j = 1; j < right.getPointCount(); j++) {
		...
		for (unsigned int i = 0; i < right.getPointCount(); i++) {
			...
			for (unsigned int j = 1; j < left.getPointCount(); j++) {
	...
	inline sf::ConvexShape shapeFixWinding(const sf::ConvexShape &shape) {
