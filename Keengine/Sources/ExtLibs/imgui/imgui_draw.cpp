...
void ImDrawList::PushClipRectFullScreen()
{
    ...
    //PushClipRect(GetVisibleRect());   //: This would be more correct but we're not supposed to access ImGuiContext from here?
}
...
void ImDrawList::AddPolyline(const ImVec2* points, const int points_count, ImU32 col, bool closed, float thickness, bool anti_aliased)
{
    ...
    if (anti_aliased)
    {
        ...
        if (!thick_line)
        {
            ...
            //: Merge the different loops, possibly remove the temporary buffer.
    ...
    else
    {
        ...
        const int vtx_count = count*4;      //: Not sharing edges
        PrimReserve(idx_count, vtx_count);
...
void ImFont::BuildLookupTable()
{
    ...
    //: Needs proper TAB handling but it needs to be contextualized (or we could arbitrary say that each string starts at "column 0" ?)
...
const char* ImFont::CalcWordWrapPositionA(float scale, const char* text, const char* text_end, float wrap_width) const
{
    ...
    //: Much possible improvements (don't cut things like "word !", "word!!!" but cut within "word,,,,", more sensible support for punctuations, support for Unicode punctuations, etc.)
...
{
    if (!text_end)
        text_end = text_begin + strlen(text_begin); //: Need to avoid this.
