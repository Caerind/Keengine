...
/*
 ...
 - getting started:
   ...
   - a typical application skeleton may be:
        ...
        //: At this points you've got a texture pointed to by 'pixels' and you need to upload that your your graphic system
        //: Store your texture pointer/identifier (whatever your engine uses) in 'io.Fonts->TexID'

        // Application main loop
        while (true)
        {
            // 1) get low-level inputs (e.g. on Win32, GetKeyboardState(), or poll your events, etc.)
            //: fill all fields of IO structure and call NewFrame
            ImGuiIO& io = ImGui::GetIO();
...
//: Replace with e.g. FNV1a hash? CRC32 pretty much randomly access 1KB. Need to do proper measurements.
...
int ImTextStrFromUtf8(ImWchar* buf, int buf_size, const char* in_text, const char* in_text_end, const char** in_text_remaining)
{
    ...
    while (buf_out < buf_end-1 && (!in_text_end || in_text < in_text_end) && *in_text)
    {
        ...
        if (c == 0)
            break;
        if (c < 0x10000)    //: Losing characters that don't fit in 2 bytes
            *buf_out++ = (ImWchar)c;
    }
    *buf_out = 0;
...
//: Need a way to reuse the result of lower_bound when doing GetInt()/SetInt() - not too bad because it only happens on explicit interaction (maximum one a frame)
...
void ImGuiTextBuffer::appendv(const char* fmt, va_list args)
{
    ...
    int len = vsnprintf(NULL, 0, fmt, args);         //: could do a first pass write attempt, likely successful on first pass.
    if (len <= 0)
        return;
...
static void SetCursorPosYAndSetupDummyPrevLine(float pos_y, float line_height)
{
    ...
    //: It is problematic that we have to do that here, because custom/equivalent end-user code would stumble on the same issue. Consider moving within SetCursorXXX functions?
...
//: Ideally we should remove the Begin/End functions but they are part of the legacy API we still support. This is why some of the code in Step() calling Begin() and reassign some fields, spaghetti style.
...
//: Add a more explicit sort order in the window structure.
...
void ImGui::Render()
{
    ...
    if (g.Style.Alpha > 0.0f)
    {
        ...
        for (int i = 0; i != g.Windows.Size; i++)
        {
            ...
            {
                //: Generalize this with a proper layering system so e.g. user can draw in specific layers, below text, ..
...
//: Note that we have a lag here because WindowRectClipped is updated in Begin() so windows moved by user via SetWindowPos() and not SetNextWindowPos() will have that rectangle lagging by a frame at the time FindHoveredWindow() is called, aka before the next Begin(). Moving window thankfully isn't affected.
static ImGuiWindow* FindHoveredWindow(ImVec2 pos, bool excluding_childs)
{
    ImGuiContext& g = *GImGui;
...
{
    //: Store sorted hashes -> pointers so we can do a bissection in a contiguous block
    ImGuiContext& g = *GImGui;
    ImGuiID id = ImHash(name, 0);
    for (int i = 0; i < g.Windows.Size; i++)
...
bool ImGui::Begin(const char* name, bool* p_open, const ImVec2& size_on_first_use, float bg_alpha, ImGuiWindowFlags flags)
{
    ...
    {
        const ImVec2 backup_cursor_pos = window->DC.CursorPos;                  //: not sure of the exact reason of this saving/restore anymore :( need to look into that.
    ...
    if (first_begin_of_the_frame)
    {
        ...
        {
            ImRect rect_to_avoid(g.IO.MousePos.x - 16, g.IO.MousePos.y - 8, g.IO.MousePos.x + 24, g.IO.MousePos.y + 24); //: Completely hard-coded. Perhaps center on cursor hit-point instead?
...
void ImGui::End()
{
    ...
    // Stop logging
    if (!(window->Flags & ImGuiWindowFlags_ChildWindow))    //: add more options for scope of logging
        LogFinish();
...
void ImGui::FocusWindow(ImGuiWindow* window)
{
    ...
    if (window->Flags & ImGuiWindowFlags_Popup) //: This statement should be unnecessary. Need further testing before removing it..
...
bool ImGui::CollapsingHeader(const char* label, bool* p_open, ImGuiTreeNodeFlags flags)
{
    ...
    if (p_open)
    {
        // Create a small overlapping close button //: We can evolve this into user accessible helpers to add extra buttons on title bars, headers, etc.
        ImGuiContext& g = *GImGui;
...
float ImGui::RoundScalar(float value, int decimal_precision)
{
    ...
    //: Investigate better rounding methods
...
#ifdef __APPLE__    //: Move setting to IO structure
...
//: The existence of this rarely exercised code path is a bit of a nuisance.
...
//: Rather messy function partly because we are doing UTF8 > u16 > UTF8 conversions on the go to more easily handle stb_textedit calls. Ideally we should stay in UTF-8 all the time. See https://github.com/nothings/stb/issues/188
bool ImGui::InputTextEx(const char* label, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* user_data)
{
    ...
    if (focus_requested || user_clicked || user_scrolled)
    {
        if (g.ActiveId != id)
        {
            ...
            //: We should probably compare the whole buffer to be on the safety side. Comparing buf (utf8) and edit_state.Text (wchar).
    ...
    if (g.ActiveId == id)
    {
        ...
        else
        {
            ...
            //: We actually always render 'buf' when calling DrawList->AddText, making the comment above incorrect.
            //: CPU waste to do this every time the widget is active, should mark dirty state from the stb_textedit callbacks.
            if (is_editable)
            {
    ...
    if (g.ActiveId == id || is_currently_scrolling)
    {
        ...
        //: This should occur on buf_display but we'd need to maintain cursor/select_start/select_end for UTF-8.
        ...
        if (edit_state.StbState.select_start != edit_state.StbState.select_end)
        {
            ...
            float bg_offy_up = is_multiline ? 0.0f : -1.0f;    //: those offsets should be part of the style? they don't play so well with multi-line selection.
...
{
    //: we could pre-compute the indices to fasten this. But only 1 active combo means the waste is limited.
...
{
    int items_count = 0;
    const char* p = items_separated_by_zeros;       //: Avoid computing this, or at least only when combo is open
    while (*p)
    {
        p += strlen(p) + 1;
        items_count++;
    ...
    if (window->Flags & (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
    {
        ...
        if (g.HoveredWindow == window && g.OpenPopupStack.Size > g.CurrentPopupStack.Size && g.OpenPopupStack[g.CurrentPopupStack.Size].ParentWindow == window)
        {
            if (ImGuiWindow* next_window = g.OpenPopupStack[g.CurrentPopupStack.Size].Window)
            {
                ...
                tb.y = ta.y + ImMax((tb.y - extra) - ta.y, -100.0f);            // triangle is maximum 200 high to limit the slope and the bias toward large sub-menus //: Multiply by fb_scale?
...
//: May want to display/ignore the alpha component in the color display? Yet show it in the tooltip.
...
void ImGui::Separator()
{
    ...
    ItemSize(ImVec2(0.0f, 0.0f)); // NB: we don't provide our width so that it doesn't get feed back into AutoFit   //: Height should be 1.0f not 0.0f ?
    if (!ItemAdd(bb, NULL))
    {
        if (window->DC.ColumnsCount > 1)
            PushColumnClipRect();
...
void ImGui::EndGroup()
{
    ...
    {
        window->DC.CurrentLineTextBaseOffset = ImMax(window->DC.PrevLineTextBaseOffset, group_data.BackupCurrentLineTextBaseOffset);      //: Incorrect, we should grab the base offset from the *first line* of the group but it is hard to obtain now.
...
void ImGui::NextColumn()
{
    ...
    PushColumnClipRect();
    PushItemWidth(GetColumnWidth() * 0.65f);  //: Move on columns setup
}
...
//: May want to remove those helpers?
